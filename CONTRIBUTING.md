...
configure beman.range_searcher to fetch them automatically via CMake FetchContent.
...
```bash
cmake -LH -S . -B build | grep "BEMAN_RANGE_SEARCHER" -C 2
```
...
### `BEMAN_RANGE_SEARCHER_INSTALL_CONFIG_FILE_PACKAGE`

Enable installing the CMake config file package. Default: `ON`.
Values: `{ ON, OFF }`.

This is required so that users of `beman.range_searcher` can use
`find_package(beman.range_searcher)` to locate the library.
